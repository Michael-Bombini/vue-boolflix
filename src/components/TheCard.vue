<template>
  <div>
    <div class="card" @mouseover.once="fetchCast">
      ...
      />
      <div class="card-body">
        <h3 class="card-title" v-if="card.title !== undefined">
          {{ card.title }}
        </h3>
        <h3 class="card-title" v-else>{{ card.name }}</h3>
        <h6 class="card-title" v-if="card.original_title !== undefined">
          {{ card.original_title }}
        </h6>
        ...
        <div v-if="cast">
          ...
          <img
            :src="`https://image.tmdb.org/t/p/original/${cast[0].profile_path}`"
            alt=""
            class="cast-icon"
          />
          <p class="text-light">{{ cast[0].character }}</p>
